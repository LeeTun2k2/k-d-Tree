...
KDNode::~KDNode() = default;
///////////// End Constructor and Destroyer ////////////////



double KDNode::coord(const size_t & idx) { return x.at(idx); }
KDNode::operator bool() { return (!x.empty()); } // Trả vector có emtpy hay không
...
KDNodePtr KDTree::nearest_(   //
    ...
    if (!further->x.empty()) {
        double dl = dist2(further->x, pt);dem+=9;
        if (dl < best_dist_l) {dem+=2;
            best_dist_l = dl;
            best_l = further;
        }dem++;
    }dem++;
...
pointIndexArr KDTree::neighborhood_(  //
    ...
    if (d <= r2) {
        nbh.push_back(pointIndex(*branch));dem++;
    }dem++;
...
KDTree Init_KD_Tree(pointVec allPoints) // Khởi tạo kd tree
{
    KDTree tree(allPoints);
    return tree;
}
point_t findNearestPoint(KDTree tree, point_t point)
{
    return tree.nearest_point(point);
}
pointVec findNeighborhood(KDTree tree, point_t point, double errorNumber)
{ 
    pointIndexArr pairNeighbor = tree.neighborhood(point, errorNumber);
    pointVec pointVecNeighbor;
    for (pointIndex element : pairNeighbor)
    {
        pointVecNeighbor.push_back(element.first);
    }
    return pointVecNeighbor;
}
